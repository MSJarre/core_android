...
class IntentService:
    def __init__(self, bus):
        ...
        self.active_skills = []  # [skill_id , timestamp, user]
    ...
    def do_converse(self, utterances, skill_id, lang, message, client_id=None):
    ...
    def add_active_skill(self, skill_id, user):
        ...
        self.active_skills.insert(0, [skill_id, time.time(), user])
    ...
    def handle_utterance(self, message):
        farm_id = message.context.get("farmid")
        client_id = message.context.get("clientid")
        ...
        try:
            ...
            with stopwatch:
                ...
                converse = self._converse(combined, lang, message, client_id)
                if not converse:
                    # No conversation, use intent system to handle utterance
                    intent = self._adapt_intent_match(utterances, norm_utterances, lang)
                    for utt in combined:
                        _intent = PadatiousService.instance.calc_intent(utt)
                        if 'EkylibreSkill' in str(_intent.name):
                            _intent.name = _intent.name.replace("EkylibreSkill", farm_id.replace(".","_"))
                        if _intent:
            ...
            if converse:
                ...
                self.add_active_skill(skill_id, client_id)
    ...
    def _converse(self, utterances, lang, message, clientid=None):
        ...
        for skill in self.active_skills:
            if skill[2] == clientid:
                if self.do_converse(utterances, skill[0], lang, message):
                    ...
                    self.add_active_skill(skill[0], clientid)
                    return True
        return False
